...
export async function feedSentimentAnalysis() {
  ...
  const positive = analyzed.filter((p: any) => p.label === 'positive').length;
  const negative = analyzed.filter((p: any) => p.label === 'negative').length;
  const neutral = analyzed.filter((p: any) => p.label === 'neutral').length;
  ...
  return {
    summary: {
      ...
      avgScore: analyzed.length ? Math.round(analyzed.reduce((s: any, p: any) => s + p.score, 0) / analyzed.length * 100) / 100 : 0,
    },
    flagged: analyzed.filter((p: any) => p.score < -0.3).sort((a: any, b: any) => a.score - b.score),
    posts: analyzed.sort((a: any, b: any) => a.score - b.score),
  };
}
