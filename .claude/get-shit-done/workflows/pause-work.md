...
```bash
timestamp=$(node "/Volumes/AdnaanSSD/Mac Download/Alumani App/.claude/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "/Volumes/AdnaanSSD/Mac Download/Alumani App/.claude/get-shit-done/bin/gsd-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
