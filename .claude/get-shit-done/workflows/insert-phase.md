...
```bash
INIT=$(node "/Volumes/AdnaanSSD/Mac Download/Alumani App/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "/Volumes/AdnaanSSD/Mac Download/Alumani App/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
